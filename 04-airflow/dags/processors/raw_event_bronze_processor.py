...
# clickhouse_utils.py sits in the dags/ root, one level up from this package,
# and Airflow 3's bundle loader does not reliably put that directory on
# sys.path. The DAG file adds it too; this repeat keeps the module importable
# on its own (Airflow's safe-mode DAG discovery imports this file standalone,
# because the word "DAG" appears in a docstring below).
sys.path.insert(
    0,
...
def extract_after_payload(raw_value: Any) -> Optional[dict]:
    ...
    CREATE and UPDATE are both handled identically.

    The operation type is intentionally not used to determine
    the Bronze operation.
    """
...
def validate_table_mapping(
    ...
    """
    Validate that every configured Bronze target column exists
    in the actual Bronze table.

    _ingested_at is intentionally excluded because it is populated
    by the ClickHouse DEFAULT expression.
    """
...
# Enough events to establish the shape of `after`. Debezium emits a full row
# image, so one would do; a sample guards against a single odd event without
# re-parsing an entire page.
...
def validate_source_keys(
    ...
    """
    Validate that every configured *source* column actually appears in the
    Debezium `after` payload.

    This is the counterpart to validate_table_mapping(), which only checks the
    Bronze side of the mapping. Without this check, a source key that does not
    exist -- a typo, or a column that was never added to the source table --
    is silently coerced to ''/0/False by coerce_value() and lands in Bronze
    looking like real data. That is the one silent-failure mode an explicit
    mapping does not remove on its own.

    Keys are collected across a sample of events rather than a single one, so
    one unusual event cannot cause a false failure.
    """
    ...
    if missing:
        raise ValueError(
            ...
            f"Fix the mapping in config/raw_event_bronze_tables.py, or remove "
...
def debezium_date_to_date32(value: Any) -> Optional[date]:
    """
    Debezium DATE values are commonly represented as an integer
    number of days since Unix epoch.

    Convert that integer to a Python date.
    """
...
def coerce_value(
    ...
    """
    Convert a Debezium value into a value suitable for
    the target Bronze ClickHouse type.

    Non-nullable Bronze columns receive explicit defaults:
    ...
    Date/Date32 NULL values are left as None so that the caller
    can fail clearly if a non-nullable date field is unexpectedly
    missing.
    """
...
def process_table(
    ...
    """
    Process one Raw -> Bronze table for one DAG window.
    """
    ...
    # Reconciliation. This pipeline only expects CREATE and UPDATE events, and
    # both carry an `after` row image -- so raw events read and Bronze rows
    # written must be equal. A mismatch means either a DELETE reached the topic
    # (the connector runs tombstones.on.delete=false and the services
    # soft-delete, so that should be impossible) or the envelope shape changed.
    # Either way it is silent data loss, so it fails the task rather than
    # leaving a warning nobody reads.
    #
    # allow_null_after escapes this per table. It exists because the failure is
    # otherwise a poison pill: the offending event stays in the window, so every
    # retry re-reads it and fails again. Re-running is safe regardless -- Bronze
    # is ReplacingMergeTree, so the rows that did land collapse on merge.
    if total_skipped and not table_config.get("allow_null_after", False):
        raise ValueError(
            ...
            f"table in config/raw_event_bronze_tables.py to accept the loss and "
            f"proceed."
        )
